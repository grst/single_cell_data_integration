...
with(azizi_peer, {
  ...
  write_tsv(mat,  "../results/data_processed/azizi_peer_2018/raw_counts.tsv")
...
with(azizi_peer_10x, {
   raw_counts = read_tsv("../data/azizi_peer_2018_10x/GSE114724_raw_counts.tsv") 
   ...
   raw_counts %>% filter(gene != "Sample_Barcodes") %>% 
     write_tsv("../results/data_processed/azizi_peer_2018_10x/raw_counts.tsv")
...
with(guo_zhang_2018, {
  ...
  raw_counts %>% select(-geneID) %>% drop_na() %>% write_tsv("../results/data_processed/guo_zhang_2018/raw_counts.tsv")
})
rm(guo_zhang_2018)
```
...
with(zheng_zhang, {
  ...
  raw_counts %>% select(-geneID) %>% drop_na() %>% write_tsv("../results/data_processed/zheng_zhang_2017/raw_counts.tsv")
})
rm(zheng_zhang)
```

```{r}
dir("../results/data_processed/", no..=TRUE, full.names = TRUE) %>% sapply(normalizePath) %>% write_lines("../results/data_processed/datasets.txt")
```
