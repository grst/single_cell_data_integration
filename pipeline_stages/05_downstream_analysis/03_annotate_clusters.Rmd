...
import itertools
from plotting import colors, plot_umap
```
...
#999999"""
{
    cluster: color.strip() for cluster, color in zip(
        sorted(cluster_names.values()), colorbrewer.strip().split("\n"))
}
```
...
```{python}
genes = iter(["GZMK", "ZNF683", "HAVCR2", "GZMB", "IL7R", 
         "STMN1", "HSPA6", "IFIT3"])
fig, axs = plt.subplots(2, 4, figsize=(14, 6))
for row in axs:
    for ax in row:
        gene = next(genes)
        plot_umap(adata, rep="X_draw_graph_fa", color=gene, ax=ax, title=gene, size=0.2)
        ax.axis("off")
...
```{python}
ax = sns.clustermap(go_tab_pivot[go_tab_pivot.index.get_level_values('go').isin(top_go)],
              col_cluster=False, figsize=(4, 8), metric="correlation")
ax.ax_heatmap.xaxis.set_ticks_position('top')
ax.ax_heatmap.set_ylabel(None)
ax.ax_heatmap.xaxis.set_ticklabels(ax.ax_heatmap.xaxis.get_ticklabels(), rotation=90)
