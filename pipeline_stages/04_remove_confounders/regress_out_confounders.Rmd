...
The `raw` data object will contain counts that are log-scaled, but not normalized. These valuese will be used later for visualization of marker genes.

```{python}
# data could be already log-scales from a previous step.
if 'is_log' not in adata.uns or not adata.uns['is_log']:
