...
```{python}
sc.pl.pca(adata, color=["n_genes", "n_counts", "percent_mito", "dataset", "patient", "origin", "platform"], ncols=2)
```
...
sc.pl.pca_scatter(adata_cc_genes, color=['phase', 'dataset', 'platform', 'n_genes'], ncols=2)
```
```{python}
