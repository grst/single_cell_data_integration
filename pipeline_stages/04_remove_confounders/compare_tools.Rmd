...
def plot_umap(adata, ax, title="umap", size=0.01, rep="X_umap", color="dataset"):
    assert len(np.unique(adata.obs[color].values)) == len(colors[color]), \
...
fig.savefig("results/data_integrated/compare_tools/umap_dataset.jpg", quality=100, dpi=300)
```
...
fig.delaxes(axs[1, 3])
fig.subplots_adjust(hspace=.4)
fig.savefig("results/data_integrated/compare_tools/umap_cell_type.jpg", quality=100, dpi=300)
```
...
fig.delaxes(axs[1, 3])
fig.subplots_adjust(hspace=.4)
fig.savefig("results/data_integrated/compare_tools/umap_patient.jpg", quality=100, dpi=300)
```

```{python}
def get_connectivities(adata, tool, neighbors_total, neighbors_patient, neighbors_dataset):
    if tool == "BBKNN":
...
connectivities = p.starmap(get_connectivities,
                           [(adata, t, 152, 4, 19) for t in tools])
```
...
patient_lisi = p.starmap(lisi, [(c, adata.obs["batch_patient"].values) for c in connectivities])
```
...
fig.savefig("results/data_integrated/compare_tools/lisi_boxplot.jpg", quality=100, dpi=300)
