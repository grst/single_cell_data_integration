...
adata_umap = sc.read_h5ad("/storage/scratch/cell_type_prediction/lambrechts6653_with_umap.h5ad")
```
...
for ct in cell_types:
    ...
    markers = ct_markers["gene_symbol"].values
    title = ["{}: {}".format(ct, m) for m in markers]
    sc.pl.umap(adata_umap, color=markers, title=title, ncols=2)
```

```{python}
sc.tl.louvain(adata_umap)
```

```{python}
sc.pl.umap(adata_umap, color=["origin", "patient", "louvain"], ncols=2)
```
...
training_data = {}
for ct in cell_types: 
    ct_markers = markers_ensg.loc[(markers_ensg["cell_type"] == ct).values, ["gene_symbol"]].drop_duplicates()
    training_data[ct] = np.max(tmp_adata.X[:, tmp_adata.var_names.isin(ct_markers["gene_symbol"])], axis=1) > 1
```
...
```{python}
cell_type_labels = np.array(["unknown"] * tmp_adata.shape[0], dtype="object")
```
...
X = tmp_adata.X[cell_type_labels != "unknown", :][:, ~tmp_adata.var_names.isin(markers_ensg["gene_symbol"])]
clf = MLPClassifier(hidden_layer_sizes=(100, 100))
```

```{python}
y_counts = {ct: np.sum(y == ct) for ct in set(y)}
...
for train, test in skf.split(X, y):
    ...
    weights_train = [y_counts[ct] for ct in y_train]
#     clf.fit(X_train, y_train, sample_weight=weights_train)
    clf.fit(X_train, y_train)
    proba = clf.predict_proba(X_test)
    y_pred = clf.classes_[np.argmax(proba, axis=1)]
    y_pred[np.max(proba, axis=1) <= .5] = "unknown"
...
```{python}
proba = clf.predict_proba(tmp_adata.X[:, ~tmp_adata.var_names.isin(markers_ensg["gene_symbol"])])
