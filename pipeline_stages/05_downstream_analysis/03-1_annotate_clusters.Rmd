...
```{python}
sc.pl.draw_graph(adata, color=["IFNG", "TNF", "IL2", "GZMA", "GZMH"])
```
...
fig.savefig("results/downstream_analysis/gene_expr_fa2.jpg", quality=100, dpi=300)
```

```{python}
genes = iter(["HAVCR2", "LAG3", "CXCL13", "LAYN", "GZMB", "PRF1", "GZMK", "ENTPD1"])
fig, axs = plt.subplots(2, 4, figsize=(14, 6))
for row in axs:
    for ax in row:
        gene = next(genes)
