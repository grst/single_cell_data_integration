...
for r in resolutions:
    sc.tl.leiden(adata, resolution=r, key_added="leiden_{}".format(r))
```

```{python}
n_clusters = [np.unique(adata.obs["leiden_{}".format(r)].values).size for r in resolutions]    
```
...
```{python}
sc.pl.draw_graph(adata, color=["leiden_{}".format(r) for r in resolutions if r <=0.3], legend_loc="on data", frameon=False)
```
...
```{python}
sc.tl.paga(adata2, groups="leiden_0.3")
sc.pl.paga(adata2, threshold=0.05, color="clusters")
```
...
```{python}
sc.pl.draw_graph(adata2, color=["clusters", "HAVCR2", "GZMK", "GZMB", "ZNF683"], legend_loc="on data")
```
...
```{python}
adata3 = adata2[~adata2.obs["clusters"].isin(["5", "7", "8"]), :].copy()
```
