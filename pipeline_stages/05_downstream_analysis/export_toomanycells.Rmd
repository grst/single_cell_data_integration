...
```{python}
sc.pl.umap(tdata3, color=["cluster_label1", "cluster_label2"])
```

```{python}
sc.tl.paga(tdata3, groups='cluster_label')
sc.pl.paga(tdata3)
```

```{python}
sc.tl.draw_graph(tdata3, init_pos='paga')
sc.pl.draw_graph(tdata3, color='cluster_label', legend_loc='on data')
```
...
```{python}
sc.tl.rank_genes_groups(tdata3, groupby="cluster_label1")
```
