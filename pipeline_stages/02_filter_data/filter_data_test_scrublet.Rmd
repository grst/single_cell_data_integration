...
```{python}
mito_genes = np.unique(pd.read_csv("tables/mitochondrial_genes.tsv", sep="\t")["Gene name"].values)
biomart = pd.read_csv("tables/biomart.tsv", sep="\t")
ribo_genes = np.unique(pd.read_csv("tables/ribosomal_genes.tsv", sep="\t", comment="#")["Gene name"].values)
```
...
sc.pl.umap(adata_vis, color=["is_doublet", "doublet_score"])
```
